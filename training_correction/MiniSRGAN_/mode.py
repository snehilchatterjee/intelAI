...
def train(args):
    ...
    generator = Generator(img_feat=3, n_feats=64, kernel_size=3, num_block=8, scale=args.scale)
    ...
    while pre_epoch < args.pre_train_epoch:
        for i, tr_data in enumerate(tqdm(loader, desc=f"Epoch {pre_epoch+1}/{args.pre_train_epoch}")):
        ...
        if pre_epoch % 2 == 0:
            torch.save(generator.state_dict(), './model/pre_trained_model_latest.pt')
            print(pre_epoch)
            print(loss.item())
            print('=========')
        ...
        if loss.item() < best_loss:
            best_loss = loss.item()
            torch.save(generator.state_dict(), './model/best_pre_trained_model.pt')
    ...
    generator.load_state_dict(torch.load('./model/pre_trained_model_latest.pt'))
    ...
    while fine_epoch < args.fine_train_epoch:
        ...
        for i, tr_data in enumerate(tqdm(loader, desc=f"Epoch {fine_epoch+1}/{args.fine_train_epoch}")):
            gt = tr_data['GT'].to(device)
            lr = tr_data['LR'].to(device)
            ...
            if g_loss.item() < best_loss:
                ...
                torch.save(generator.state_dict(), './model/best_trained_model.pt')
                print(f"New best model saved with loss: {best_loss}")
        ...
        if fine_epoch % 2 == 0:
            ...
            print(d_loss.item())
            print('=========')
            torch.save(generator.state_dict(), './model/latest_trained_model.pt')
...
def test(args):
    ...
    generator = Generator(img_feat=3, n_feats=64, kernel_size=3, num_block=8)
    generator.load_state_dict(torch.load(args.generator_path))
    generator = generator.to(device)
    generator.eval()
